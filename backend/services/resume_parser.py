...
from backend.utils.file_utils import(
  FileParsingError, TextExtractionError, FileUploadError,
  log_error, log_warning, log_info, with_fallback
)

from backend.core.config import (
    MAX_FILE_SIZE_MB, SUPPORTED_MIME_TYPES, MAX_FILE_SIZE_BYTES
)
...
def extract_text_from_pdf(file_data: bytes) -> str:
    try:
        ...
        if used_fallback:
            log_info(
                'Used PyPDF2 fallback method for PDF text extraction.',
...
def parse_resume_file(file_data: bytes, filename: str)-> Tuple[str, dict]:
    ...
    try:
        ...
        if not is_valid:
            log_warning(f'File validation failed for file {filename}', context='parse_resume_file')
